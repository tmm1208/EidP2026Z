...
Die Summe aller geraden Zahlen von 2 bis 100.
Die Summe aller ungeraden Zahlen von 1 bis 99.
Die Summe aller Zahlen, die durch 5 teilbar sind, von 5 bis 50.
Lernziele:
...
Wörter mit weniger als 4 Buchstaben werden übersprungen (`continue`).
Sobald das Wort `"stopp"` auftaucht, wird die Schleife sofort beendet (`break`).
...
Alle Zahlen von der Eingabe bis 1 rückwärts ausgibt (mit `while`-Schleife, alle in einer Zeile).
Danach `"Start!"` ausgibt.
Anschließend die Quersumme der eingegebenen Zahl berechnet und ausgibt (mit `for`-Schleife).
Lernziele:
...
Der Benutzer wird in einer `while`-Schleife nach Artikeln gefragt, bis er `"fertig"` eingibt.
Jeder eingegebene Artikel wird einer Liste hinzugefügt (nutze `.append()`).
Artikel, die kürzer als 2 Zeichen sind, werden mit `continue` übersprungen und eine Hinweismeldung ausgegeben.
Nach Abschluss wird die Liste nummeriert mit einer `for`-Schleife ausgegeben.
Zum Schluss wird die Gesamtanzahl der Artikel ausgegeben.
Lernziele:
